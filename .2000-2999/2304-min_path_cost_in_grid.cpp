...
int minPathCost(vector<vector<int>>& grid, vector<vector<int>>& moveCost)
{
	...
	vector<int> pre(cols), cur = grid[0];
	for (int h = 1; h < rows; ++h) {
		pre.swap(cur);
		for (int w = 0; w < cols; ++w) {
			cur[w] = INT_MAX;
			for (int x = 0; x < cols; ++x)
				cur[w] = min(cur[w], pre[x] + moveCost[grid[h - 1][x]][w]);
			cur[w] += grid[h][w];
		}
	}
...
int main()
{
	vector<vector<int>>
		g1 = {{5, 3}, {4, 0}, {2, 1}},
		m1 = {{9, 8}, {1, 5}, {10, 12}, {18, 6}, {2, 4}, {14, 3}},
		g2 = {{5, 1, 2}, {4, 0, 3}},
		m2 = {{12, 10, 15}, {20, 23, 8}, {21, 7, 1}, {8, 1, 13}, {9, 10, 25}, {5, 3, 2}};
	ToOut(minPathCost(g1, m1));
	ToOut(minPathCost(g2, m2));
}
