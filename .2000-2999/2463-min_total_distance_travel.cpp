...
每个机器人所在的位置 互不相同 。每个工厂所在的位置也 互不相同 。
...
{
	int nbRobot = static_cast<int>(robot.size());
	std::sort(robot.begin(), robot.end());
	std::sort(factory.begin(), factory.end());
	vector<long long> dp(nbRobot + 1, LLONG_MAX / 2);
	dp[0] = 0;
	for (auto& f : factory) {
		int pos = f[0], limit = f[1];
		for (int i = nbRobot; i > 0; --i) {
			long long cost = 0;
			for (int k = 1, kend = min(i, limit); k <= kend; ++k) {
...
int main()
{
	...
	vector<vector<int>> factory1 = {{2, 2}, {6, 2}},
											factory2 = {{-2, 1}, {2, 1}};
	ToOut(minimumTotalDistance(robot1, factory1));
	ToOut(minimumTotalDistance(robot2, factory2));
}
