...
public:
	...
	vector<int> withdraw(int64_t amount)
	{
		vector<int> ans(5), cannot(1, -1);
		for (int i = 4; i >= 0; --i) {
			int64_t k = min(amount / money[i], count[i]);
			ans[i] = static_cast<int>(k);
			amount -= k * money[i];
		}
		if (amount) return cannot;
		for (int i = 0; i < 5; ++i)
			count[i] -= ans[i];
		return ans;
	}
...
int main()
{
	...
	ToOut(atm.withdraw(600));
	atm.deposit({0, 1, 0, 1, 1});
	ToOut(atm.withdraw(600));
	ToOut(atm.withdraw(550));
}
