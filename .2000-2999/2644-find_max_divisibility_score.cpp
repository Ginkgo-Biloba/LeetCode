...
int maxDivScore(
	vector<int> const& A, vector<int> const& D)
{
	int maxnum = 0, maxval = INT_MAX;
	for (int val : D) {
		int num = 0;
		for (int a : A)
			num += a % val == 0;
		if (num > maxnum || (num == maxnum && val < maxval)) {
			maxnum = num;
			maxval = val;
		}
	}
...
int main()
{
	ToOut(maxDivScore({2, 9, 15, 50}, {5, 3, 7, 2}));
	ToOut(maxDivScore({4, 7, 9, 3, 9}, {5, 2, 3}));
	ToOut(maxDivScore({20, 14, 21, 10}, {10, 16, 20}));
}
