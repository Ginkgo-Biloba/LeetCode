...
int numberOfWays(int startPos, int endPos, int k)
{
	...
	if (k < d || (k - d) % 2)
		return 0;
	int r = (k - d) / 2;
	vector<array<int, 1024>> comb(k + 1);
	memset(comb.data(), 0, sizeof(comb[0]) * comb.size());
	comb[0][0] = 1;
	for (int n = 1; n <= k; ++n) {
		comb[n][0] = 1;
		for (int i = 1; i <= r; ++i)
...
int main()
{
	ToOut(numberOfWays(1, 2, 3));
	ToOut(numberOfWays(2, 5, 10));
	ToOut(numberOfWays(272, 270, 6));
}
