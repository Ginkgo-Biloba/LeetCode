...
int countTexts(string keys)
{
	...
	dp[0] = dp[1] = 1;
	for (int i = 1; i < n; ++i) {
		int rep = (keys[i] == '7' || keys[i] == '9') ? 4 : 3;
		int backto = max(0, i - rep + 1);
		dp[i + 1] = dp[i];
		for (int k = i - 1; k >= backto; --k) {
			if (keys[k] != keys[i])
				break;
...
int main()
{
	ToOut(countTexts("22233"));
	ToOut(countTexts("222222222222222222222222222222222222"));
}
