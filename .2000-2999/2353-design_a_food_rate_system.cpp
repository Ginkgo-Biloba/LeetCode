...
public:
	FoodRatings(
		vector<string> const& foods,
		vector<string> const& cuisines,
		vector<int> const& ratings)
	{
		size_t n = foods.size();
	...
	{
		auto& rc = Frc[food];
		auto& S = Crf[rc.second];
		S.erase({-rc.first, food});
...
int main()
{
	FoodRatings f(
		...
		{9, 12, 8, 15, 14, 7});
	ToOut(f.highestRated("korean"));
	ToOut(f.highestRated("japanese"));
